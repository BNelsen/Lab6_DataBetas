...
DENflights_total <- COflights %>% filter(DEST == 'DEN') %>% summarise('total' = n())
late_arrival_total <- DENflights %>% filter(DENflights$ARR_DELAY >= 15) %>% summarise('late' = n())
PLateArr <- late_arrival_total/DENflights_total
...
late_departure_total <- DENflights %>% filter(DEP_DELAY >= 15) %>% summarise('departure' = n())
PLateDep <- late_departure_total/DENflights_total
...
ggplot(data = TopOriginFlight) + geom_col(mapping = aes(x = ORIGIN, y = total, fill = ORIGIN)) +
  ...
  ylab('Total Count')+
  ggtitle('Total Delayed Flights to DEN by Origin of Departure')
```
***Drew's Plot:***
